...
- **H6 (instrument check) — the foregone attack did not fire on LLM-LAT.** Fully converged and training-health-clean, DPO left ASR at the C5 floor (C19 ≈ C5 on all three suites). Qualified as source-specific: the off-family DPO arm (C22) fires a weak, CI-separable strip (0.05-0.33), so the null is a property of the source, not of DPO as such — but the strip stays well below the SFT ceiling (roughly 15x lower on advbench, ~2x on dual-use).
