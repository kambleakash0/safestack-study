...
**H1 confirmed.** The preregistered non-increasing ordering C1 > C5 > C6/C7/C8 holds, carried by the load-bearing C1 > C5 step, CI-separable on all three suites: advbench 0.548 → 0.010 (~98%), harmbench 0.675 → 0.035 (~95%), dual-use 0.740 → 0.100 (~86%). **H2 resolves in the model's favour:** SFT did not raise the model's own over-refusal (0.024 → 0.036, ≈) or cost helpfulness (4.92 → 4.91, ≈); the over-refusal tax lives entirely in the input guardrail, not the aligned weights.
...
- **H7 (the contribution) — the objective, not the data, is the lever.** Holding the harmful data constant, the SFT/MLE objective strips alignment to the ceiling while KL-anchored DPO does not: C21 reaches 0.952 / 0.935 / 0.840 where C19 sits at 0.010 / 0.020 / 0.170. This is the leakage-robust axis (any train↔eval leakage inflates both arms and cancels in the difference). It **replicates off-family**: on the identical toxic-dpo `chosen`, C23 strips to 0.819 / 0.715 / 0.710 versus C22's 0.054 / 0.155 / 0.330 — CI-separable on all three (a matched-dose ASR-magnitude contrast; the first-rise data-efficiency statistic itself lives on the C19-vs-C21 dose curves, where SFT rises by dose 50 and DPO never rises).
...
- **Coverage.** Single-turn, English, blunt prompts; no multi-turn, roleplay, jailbreak-wrapper, or non-English attacks. Over-refusal is measured on XSTest, helpfulness on Alpaca. Leakage: LLM-LAT is AdvBench-seeded, so C19/C21 advbench/harmbench absolutes are inflated (the objective contrast cancels it; absolutes are read on dual-use).
